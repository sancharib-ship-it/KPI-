...
export const WaveComparison: React.FC<WaveComparisonProps> = ({ region }) => {
  ...
  return (
    <div className="px-6 py-4">
      <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">Wave Comparison</h2>
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={data} margin={{ top: 10, right: 30, bottom: 10, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
            <XAxis dataKey="label" tick={{ fontSize: 11 }} />
            <YAxis tick={{ fontSize: 11 }} label={{ value: "Lift (pts)", angle: -90, position: "insideLeft", fontSize: 11, fill: "#6b7280" }} />
            <Tooltip formatter={(v: number | undefined, name: string | undefined) => [`${(v ?? 0).toFixed(1)} pts`, name ?? ""]} />
            <Legend />
            <Bar dataKey="w1" name="Wave 1" fill="#3b82f6" radius={[4, 4, 0, 0]} />
            <Bar dataKey="w2" name="Wave 2" fill="#8b5cf6" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
        <div className="mt-2 flex flex-wrap gap-3">
          {data.map(({ label, target }) => {
            ...
            return (
              <div key={label} className="text-xs text-gray-500 flex items-center gap-1">
